import { A as APIServices } from "./index-DlTQ3KJj.js";
...
function transformPeerAssessmentsForUI(rawAssessments, employees = [], currentEmployeeId = null) {
  ...
  const outstanding = items.filter((a) => (a.assessorID === myId || a.assessor_id === myId) && (a.status === "pending" || a.status === "draft")).map((a) => ({
    ...
    assesseeID: a.assesseeID || a.assessee_id || a.employee_id || null,
    employeeID: a.employee_id || a.assesseeID || null,
    managerID: a.manager_id || a.managerID || null,
...
function transformManagerAssessmentsForUI(rawAssessments, employees = [], currentEmployeeId = null) {
  ...
  const received = items.filter((a) => a.employee_id === myId || a.assesseeID === myId).map((a) => ({
    ...
    assesseeID: a.assesseeID || a.employee_id || null,
    employeeID: a.employee_id || a.assesseeID || null,
    managerID: a.manager_id || a.managerID || a.assessorID || null,
    relationship: a.relationship || null,
...
async function loadInitialData() {
  ...
  if (currentEmployee == null ? void 0 : currentEmployee.deptcode) {
    ...
    try {
      const deptsResp = await svc.getAllDepartments();
      const depts = Array.isArray(deptsResp) ? deptsResp : deptsResp && deptsResp.departments ? deptsResp.departments : [];
      const deptMap = (depts || []).reduce((m, d) => {
        const code = d.deptcode || d.code || d.sno || d.id;
        if (code) m[code] = d.deptname || d.name || d.label || "";
        return m;
      }, {});
      colleagues = colleagues.map((e) => ({
        ...e,
        department: e.department || e.deptname || deptMap[e.deptcode] || ""
      }));
    } catch (error) {
      console.warn("Failed to load departments for colleague department names:", error);
    }
  }
