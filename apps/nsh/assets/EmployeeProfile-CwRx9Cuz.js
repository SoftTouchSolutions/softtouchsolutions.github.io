...
import { ag as useParams, f as PageHeaders, R as Row, C as Col, g as Card, n as CardHeader, h as CardBody, I as Input, L as Label, A as APIServices, S as Swal } from "./index-DlTQ3KJj.js";
...
import { g as getProfilePictureUrl } from "./profilePictureHelper-BH3TDmix.js";
...
const EmployeeProfile = () => {
  ...
  const [profilePicture, setProfilePicture] = reactExports.useState(user1);
  ...
  reactExports.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        try {
          const profile = await APIServices.getUserProfile();
          if (profile && (profile.userType || profile.role)) userRole = profile.userType || profile.role;
        } catch (e) {
        }
    ...
    const fetchEmployeeData = async () => {
      try {
        ...
        if (employeeId) {
          ...
          console.log("No employeeId provided, fetching current employee profile");
          try {
            const employeeData = await APIServices.getCurrentEmployee();
            if (employeeData) {
              console.log("Setting form data with employee data:", employeeData);
              populateFormData(employeeData);
              return;
            }
          } catch (empError) {
            console.log("getCurrentEmployee failed, trying getUserProfile:", empError);
          }
          const userData = await APIServices.getUserProfile();
          if (userData) {
            console.log("Setting form data with user profile data:", userData);
            let employeeData = null;
            if (userData.employeeID) {
              try {
                employeeData = await APIServices.getEmployee(userData.employeeID);
              } catch (e) {
                console.log("Could not fetch employee by ID from user data");
              }
            }
            const mergedData = {
              ...userData,
              ...
              staff_name: (employeeData == null ? void 0 : employeeData.staff_name) || (employeeData == null ? void 0 : employeeData.firstname) ? `${employeeData.firstname || ""} ${employeeData.lastname || ""}`.trim() : userData.name || "",
              email: userData.email || (employeeData == null ? void 0 : employeeData.email) || "",
              employeeID: userData.employeeID || (employeeData == null ? void 0 : employeeData.employeeID) || "",
              userId: userData.userID || userData._id || userData.id || ""
            };
            populateFormData(mergedData);
          }
        }
      } catch (error) {
    ...
    const populateFormData = (data) => {
      setFormData((prev) => {
        ...
        return {
          ...prev,
          title: data.title || "",
          staff_name: data.staff_name || (data.firstname ? `${data.firstname} ${data.lastname || ""}`.trim() : ""),
          email: data.email || "",
          userId: data.userID || data._id || data.id || "",
          employeeID: data.employeeID || "",
          phonenumber: data.phonenumber || data.phone || "",
          address: data.address || "",
          deptcode: data.deptcode || ((_a = data.department) == null ? void 0 : _a.deptcode) || "",
          deptname: data.deptname || ((_b = data.department) == null ? void 0 : _b.deptname) || "",
          managerId: data.managerId || data.manager_id || "",
          managername: data.managername || ((_c = data.manager) == null ? void 0 : _c.staff_name) || "",
  ...
  reactExports.useEffect(() => {
    const loadUserData = async () => {
      try {
        ...
        const userData = (response == null ? void 0 : response.data) || response;
        if (userData) {
          console.log("Profile picture found:", userData.profile_picture);
          setProfilePicture(getProfilePictureUrl(userData.profile_picture, user1));
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      if (response) {
        const pictureValue = typeof response === "string" ? response : response.path;
        setProfilePicture(getProfilePictureUrl(pictureValue, user1));
      }
