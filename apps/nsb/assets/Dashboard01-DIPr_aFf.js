...
import { y as mapToCssModules, z as classnames, E as tagPropType, H as useDispatch, J as ServiceFactory, w as Card, K as CardHeader, x as CardBody, Q as Spinner, T as Alert, U as Table, V as PageHeader, R as Row, C as Col, W as Progress, X as Badge, q as Link, r as DropdownToggle, D as DropdownMenu, A as APIServices } from "./index-DTmWxS77.js";
...
function computeCriteriaStats(assessments, criteriaLookup = {}) {
  ...
  assessments.forEach((a) => {
    ...
    const relationship = a.relationship || "SELF";
    if (category === "Task Execution") {
      stats.taskExecution.count++;
      stats.taskExecution.avg += rating;
      if (relationship === "SELF") stats.taskExecution.self = rating;
      if (relationship === "MANAGER") stats.taskExecution.manager = rating;
    } else if (category === "Power Skills") {
      stats.powerSkills.count++;
      stats.powerSkills.avg += rating;
      if (relationship === "SELF") stats.powerSkills.self = rating;
      if (relationship === "MANAGER") stats.powerSkills.manager = rating;
    } else if (category === "Attitude") {
      stats.attitude.count++;
      stats.attitude.avg += rating;
      if (relationship === "SELF") stats.attitude.self = rating;
      if (relationship === "PEER") stats.attitude.peer = rating;
    }
  });
